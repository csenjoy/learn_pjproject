...
PJ_DEF(pj_status_t) pjmedia_h264_packetize(pjmedia_h264_packetizer *pktz,
					   pj_uint8_t *buf,
                                           pj_size_t buf_len,
                                           unsigned *pos,
                                           const pj_uint8_t **payload,//payload指向buf
                                           pj_size_t *payload_len)
{
    ...
    /* Get end of NAL unit */
    p = nal_start+pktz->cfg.mtu+1;//+1 for ignore nal head
    if (p > end || pktz->cfg.mode==PJMEDIA_H264_PACKETIZER_MODE_SINGLE_NAL) 
	p = end;//p can not greater than end
    nal_end = find_next_nal_unit(nal_start, p);//find nal unit between nal_start and p 
    if (!nal_end)
	nal_end = p;//only one nal unit
	...
	//FU-A packetize时，首次占用start code与nal header字段
	//下次时会覆盖nal数据部分，但是首次调用时，已经得到nal数据了，所以覆盖不会有影响
	//对nalu单元分包处理逻辑：扫描处理，已扫描部分处理完成
	p = nal_start - HEADER_SIZE_FU_A;//Override nal header and 1 byte of start code
	*p = (NRI << 5) | NAL_TYPE_FU_A;//Format FU Indicator
	++p;
					     ...
					     unsigned   *bits_pos)
{
    ...
    nal_start_code = nal_start + PJ_ARRAY_SIZE(nal_start) -
    		     pktz->cfg.unpack_nal_start;//nal start code len 4 or 3
    ...
    {
	/* Single NAL unit packet */
	pj_uint8_t *p = bits + *bits_pos;//seek to free buffer pos of bits
	...
	*bits_pos = (unsigned)(p - bits);
	...
	/* Fill bitstream */
	p = bits + *bits_pos;//seek to free buffer pos of bits
	p_end = bits + bits_len;//bits buffer end pos
	q = payload + 1;//skip stap-a nal head
	q_end = payload + payload_len;//nalu buffer end pos
	while (q < q_end && p < p_end) {
	    ...
	    /* Get NAL unit size */
	    tmp_nal_size = (*q << 8) | *(q+1);//read 2 bytes for nalu size
	    q += 2;//skip nalu size  
	...
	pj_uint8_t *p;
	const pj_uint8_t *q = payload;//q start of payload
	...
	//read FU Header(q+1)
	/* Get info */
	S = *(q+1) & 0x80;    /* Start bit flag	at first bit*/
	E = *(q+1) & 0x40;    /* End bit flag	at second bit*/
	TYPE = *(q+1) & 0x1f;/*4-8bit for nalu type*/
	NRI = (*q & 0x60) >> 5;/*nal_refer_idc*/
	...
	q += 2;//Skip 1 byte FU Indicator and 1 byte FU Head
