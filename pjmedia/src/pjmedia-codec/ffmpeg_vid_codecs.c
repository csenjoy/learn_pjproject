					...
					pjmedia_frame *output)
{
	...
	for (i=0; i<pkt_count; ++i) {
        //whole_len为ffmpeg_unpacketize[IN|OUT]参数，记录已unpacketize字节流长度
        //也是ff->dec_buf已使用buffer长度
	    if (whole_len + packets[i].size > ff->dec_buf_size) {
		PJ_LOG(5,(THIS_FILE, "Decoding buffer overflow"));
		break;
	    }
        //将packets[i].buf中的的数据unpacketize到dec_buf中
        //ffmpeg_unpacketize函数使用了&whole_len，记录dec_buf当前空闲buffer pos
